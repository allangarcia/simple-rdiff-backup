#!/bin/bash
#
# Postgresql database list util of simple rdiff-backup script by Allan Garcia
#
...
ssh -p ${PORT} root@${ADDR} "sudo -i -u postgres psql -q -A -t -c 'SELECT datname FROM pg_database'"
