#!/bin/bash
#
# Postgresql dump remote of simple rdiff-backup script by Allan Garcia
#
...
if [[ $# -ne 3 ]]; then
    ...
    echo "Usage: $0 <addr> <port> <database>"
    exit 1
fi
...
ssh -p ${PORT} root@${ADDR} "sudo -i -u postgres pg_dump ${DATABASE}"
